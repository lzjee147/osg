            ...
            for(unsigned int i=0;i<_rangeList.size();++i)
            {
                if (_rangeList[i].first<=required_range && required_range<_rangeList[i].second)
                {
                    if (i<_children.size())
                    {
						...
						osgUtil::CullVisitor* cullStack = dynamic_cast<osgUtil::CullVisitor*>(&nv);
            ...
            if (needToLoadChild)
            {
                    ...
                    numChildren<_perRangeDataList.size())
                {
                    ...
                    else
                    {
						...
						//nv.getDatabaseRequestHandler()->requestNodeFile("L://ºþÄÏõ·ÁêÒ½Ôº¡ªMEP-1F//" + _perRangeDataList[numChildren]._filename,nv.getNodePath(),priority,nv.getFrameStamp(), _perRangeDataList[numChildren]._databaseRequest, _databaseOptions.get());
