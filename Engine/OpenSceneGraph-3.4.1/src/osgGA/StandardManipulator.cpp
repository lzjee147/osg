...
osg::Matrixd BimsetTransformation( const osg::Vec3d& eye, const osg::Vec3d& center, const osg::Vec3d& up )
{
	Vec3d lv( center - eye );
	...
	osg::Vec3d _center = center;
	float _distance = lv.length();
...
void StandardManipulator::home( double /*currentTime*/ )
{
	...
	//_viewMatrix = BimsetTransformation( _homeEye, _homeCenter, _homeUp);
...
void StandardManipulator::home( const GUIEventAdapter& /*ea*/, GUIActionAdapter& us )
{
	...
	_viewMatrix.makeLookAt(_homeEye, _homeCenter, _homeUp);
...
{
	 _camera = us.asView() ? us.asView()->getCamera() : NULL;
