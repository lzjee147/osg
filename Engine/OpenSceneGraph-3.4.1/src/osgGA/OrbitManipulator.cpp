...
bool OrbitManipulator::handleFrame( const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& us )
{
	if(_centers.size() > 0 && _centeri < 11)
	{
		_center = _centers[_centeri];
		_distance = _distances[_centeri];

		_rotation = _rotations[_centeri];
...
osg::Quat GetQuat(osg::BoundingSphere sphere)
{
...
void OrbitManipulator::setAnimalModelBS(osg::BoundingSphere sphere)
{
	_centers.clear()   ;
	_distances.clear() ;
	_rotations.clear() ;
	...
	osg::Vec3d v = (center - _center) * 0.1;
	double d = (radius * 2 - _distance) * 0.1;

	//计算四元数
	osg::Quat quat = GetQuat(sphere);

	for (size_t i = 0; i < 11; i++)
	{
		_centers.push_back(_center + v * i)    ;
		_distances.push_back(_distance + d * i);

		osg::Quat q;

		q.slerp(i * 0.1, _rotation, quat);

		_rotations.push_back(q);
	}
}

void OrbitManipulator::Ainmal()
{
