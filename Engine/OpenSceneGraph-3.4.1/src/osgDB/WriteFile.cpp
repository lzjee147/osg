...
bool osgDB::writeFourthNodeFile(const FourthNode& fourthNode,const std::string& filename, const Options* options )
{
	ReaderWriter::WriteResult wr = Registry::instance()->writeFourthNode( fourthNode, filename, options );
	if (wr.error()) OSG_WARN << "Error writing file " << filename << ": " << wr.message() << std::endl;
	return wr.success();
}
...
bool osgDB::writeElement(const osg::Element& node, const std::string& filename, const Options* options )
{
	ReaderWriter::WriteResult wr = Registry::instance()->writeElement( node, filename, options );
	if (wr.error()) OSG_WARN << "Error writing file " << filename << ": " << wr.message() << std::endl;
	return wr.success();
}
