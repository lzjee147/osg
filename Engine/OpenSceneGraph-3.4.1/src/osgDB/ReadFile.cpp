...
OctreeBuilder* osgDB::readFourthNodeFile(const std::string& filename,const Options* options)
{
	ReaderWriter::ReadResult rr = Registry::instance()->readNode(filename,options);
	return rr.takeFourNode();
}

Element* osgDB::readElement(const std::string& filename,const Options* options)
{
	ReaderWriter::ReadResult rr = Registry::instance()->readNode(filename,options);
	return rr.GetElement();
}
