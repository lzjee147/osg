...
ReaderWriter::WriteResult Registry::writeFourthNodeImplementation(const FourthNode& image,const std::string& fileName,const Options* options)
{
	// record the errors reported by readerwriters.
	typedef std::vector<ReaderWriter::WriteResult> Results;
	Results results;
	...
	for(;itr.valid();++itr)
	{
		ReaderWriter::WriteResult rr = itr->writeFourthNode(image,fileName,options);
		if (rr.success()) return rr;
		else results.push_back(rr);
	}
...
ReaderWriter::WriteResult Registry::writeElementImplementation(const osg::Element& node,const std::string& fileName,const Options* options)
{
	// record the errors reported by readerwriters.
	typedef std::vector<ReaderWriter::WriteResult> Results;
	Results results;
	...
	for(;itr.valid();++itr)
	{
		if (itr->supportedExtensions().size()>0)
		{
			if (itr->supportedExtensions().begin()->first == "bimele")
			{
				ReaderWriter::WriteResult rr = itr->writeElement(node,fileName,options);
