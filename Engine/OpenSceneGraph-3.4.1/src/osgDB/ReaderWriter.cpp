...
OctreeBuilder* ReaderWriter::ReadResult::takeFourNode() { OctreeBuilder* node=dynamic_cast<OctreeBuilder*>(_object.get()); if (node) { node->ref(); _object=NULL; node->unref_nodelete(); } return node; }
osg::Element* ReaderWriter::ReadResult::GetElement() { osg::Element* node=dynamic_cast<osg::Element*>(_object.get()); if (node) { node->ref(); _object=NULL; node->unref_nodelete(); } return node; }
