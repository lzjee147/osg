...
public:
    ...
    virtual WriteResult writeNode(const osg::Node& node,std::ostream& fout,const Options* options =NULL) const
    {
		...
		osg::Image* image  = (osg::Image*)options->getPluginData("image");

		float* total = (float*)options->getPluginData("total");
...
osg::Program* createShader()
{
	...
	pgm->addShader(osg::Shader::readShaderFile(osg::Shader::VERTEX, osgDB::findDataFile("D://shader//lod.vert")));
	pgm->addShader(osg::Shader::readShaderFile(osg::Shader::FRAGMENT, osgDB::findDataFile("D://shader//lod.frag")));
...
osgDB::ReaderWriter::ReadResult ReaderWriterOBJ::readNode(const std::string& file, const osgDB::ReaderWriter::Options* options) const
{
	...
	osg::StateSet* dstate = geode->getOrCreateStateSet();
	dstate->setTextureAttributeAndModes(0, depthTexture, osg::StateAttribute::ON);

	dstate->setAttribute( createShader() );

	   osg::Uniform* u_anim1( new osg::Uniform( "total", total ) ); 
	  
	   dstate->addUniform( u_anim1 );
	  
	   osg::Uniform* texUniform = new osg::Uniform(osg::Uniform::SAMPLER_2D, "texA"); 
	   texUniform->set(0);
	   dstate->addUniform( texUniform );
