...
FbxMaterialToOsgStateSet::convert(const FbxSurfaceMaterial* pFbxMat)
{
    ...
    pOsgMat->setName(StringTools::FbxStr2StdStr(pFbxMat->GetName()));
...
{
    ImageMap::iterator it = _imageMap.find(StringTools::FbxStr2StdStr(fbx->GetFileName()));
    if (it != _imageMap.end())
        return it->second;
    ...
    if ((pImage = osgDB::readRefImageFile(osgDB::concatPaths(_dir, StringTools::FbxStr2StdStr(fbx->GetFileName())), _options)) ||                // First try "export dir/name"
        (pImage = osgDB::readRefImageFile(StringTools::FbxStr2StdStr(fbx->GetFileName()), _options)) ||                                        // Then try  "name" (if absolute)
        (pImage = osgDB::readRefImageFile(osgDB::concatPaths(_dir, StringTools::FbxStr2StdStr(fbx->GetRelativeFileName())), _options)))        // Else try  "current dir/name"
    {
        ...
        _imageMap.insert(std::make_pair(StringTools::FbxStr2StdStr(fbx->GetFileName()), pOsgTex.get()));
        return pOsgTex;
    }
    else
    {
