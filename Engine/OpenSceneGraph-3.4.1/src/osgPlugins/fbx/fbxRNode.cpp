    ...
    std::map<FbxNode*, osg::Node*>& nodeMap, FbxScene& fbxScene)
{
    if (bNeedSkeleton)
    {
        ...
        osgBone->setName(StringTools::FbxStr2StdStr(pNode->GetName()));
    ...
    else
    {
        ...
        if (!bAnimated && localMatrix.isIdentity())
        {
            ...
            pGroup->setName(StringTools::FbxStr2StdStr(pNode->GetName()));
            return pGroup;
        }

        osg::MatrixTransform* pTransform = new osg::MatrixTransform(localMatrix);
        pTransform->setName(StringTools::FbxStr2StdStr(pNode->GetName()));
    ...
    bool& bIsBone, int& nLightCount)
{
    ...
    std::string animName = readFbxAnimation(pNode, (StringTools::FbxStr2StdStr(pNode->GetName())).c_str());
