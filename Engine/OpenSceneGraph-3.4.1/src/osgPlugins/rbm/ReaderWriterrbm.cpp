...
class ReaderWriterrbm : public osgDB::ReaderWriter
{
public:
    ReaderWriterrbm()
    {
        supportsExtension("rbm","Alias Wavefront rbm format");
		...
		osg::Geode* geode = (osg::Geode*)osgDB::readNodeFile("D://state10.ive");
		
		state_set10_  = geode->getStateSet();

		geode = (osg::Geode*)osgDB::readNodeFile("D://state100.ive");

		state_set100_  = geode->getStateSet();

	
    }

	osg::StateSet* state_set10_ ;
	osg::StateSet* state_set100_;
    ...
    virtual WriteResult writeNode(const osg::Node& node,std::ostream& fout,const Options* options =NULL) const
    {
		...
		int* total = (int*)options->getPluginData("num");
...
inline osg::Vec3 ReaderWriterrbm::transformVertex(const osg::Vec3& vec, const bool rotate) const
{
    if(rotate==true)
    {
...
inline osg::Vec3 ReaderWriterrbm::transformNormal(const osg::Vec3& vec, const bool rotate) const
{
    if(rotate==true)
    {
        return osg::Vec3(vec.x(),-vec.z(),vec.y());
...
REGISTER_OSGPLUGIN(rbm, ReaderWriterrbm)


ReaderWriterrbm::ObjOptionsStruct ReaderWriterrbm::parseOptions(const osgDB::ReaderWriter::Options* options) const
{
...
class DrawCallBackTex : public Drawable::DrawCallback
{
public:
   GLuint textureId;
	osg::Image* image;
	...
	virtual void drawImplementation(osg::RenderInfo& renderInfo,const osg::Drawable* drawable) const
	{
		...
		//if(!image)
		//{
		//	osg::Image* image = osgDB::readImageFile("D://4.png");
		...
		//	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, image->s(), image->t(), 
...
osgDB::ReaderWriter::ReadResult ReaderWriterrbm::readNode(const std::string& file, const osgDB::ReaderWriter::Options* options) const
{
	...
	//geometry->setDrawCallback(new DrawCallBackTex());

	int num = in.readInt();

	if (num == 10)
	{
		geometry->setStateSet(state_set10_);
	}
	else if (num = 100)
	{
		geometry->setStateSet(state_set100_);
	}
	...
	//if (num == 10)
	//{
	//	geometry->setName("num10");
	//}
	//else
	//{
	//	geometry->setName("num100");
	//}
...
osgDB::ReaderWriter::ReadResult ReaderWriterrbm::readNode(std::istream& fin, const Options* options) const
{
