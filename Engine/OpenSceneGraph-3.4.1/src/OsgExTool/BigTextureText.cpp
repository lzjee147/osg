...
BigTextureText::BigTextureText()
{
   // _supportsVertexBufferObjects = true;
	geodes_ = new osg::Group();
}
...
osg::ref_ptr<osg::Geode> BigTextureText::createBigGeode()
{
	return MergeGeometryVisitor::GetMergeGeometry(geodes_);
}

void BigTextureText::addText(String text)
{
	_text = text;
	...
	String::iterator it = _text.begin();

	for(it; it != _text.end(); it++)
	{
		unsigned int charcode = *it;

		Glyph* glyph = activefont->getGlyph(_fontSize, charcode);

		//osgDB::writeImageFile(*glyph, "D://kkk.png");
		//int d = 0;
		geodes_->addChild( DebugTool::DebugFunction::CreateQuad(glyph) );
	}
}

void BigTextureText::setFont(osg::ref_ptr<Font> font)
{
...
//
Font* BigTextureText::getActiveFont()
{
    return _font.valid() ? _font.get() : Font::getDefaultFont().get();
}

const Font* BigTextureText::getActiveFont() const
{
    return _font.valid() ? _font.get() : Font::getDefaultFont().get();
}

String::iterator BigTextureText::computeLastCharacterOnLine(osg::Vec2& cursor, String::iterator first,String::iterator last)
{
    //Font* activefont = getActiveFont();
    ...
    //      }
    //}
    String::iterator lastChar;
    
    return lastChar;
}


void BigTextureText::computeGlyphRepresentation()
{
    //Font* activefont = getActiveFont();
    //if (!activefont) return;
...
bool BigTextureText::computeAverageGlyphWidthAndHeight(float& avg_width, float& avg_height) const
{
    float width = 0.0f;
    float height = 0.0f;
...
void BigTextureText::computePositions(unsigned int contextID) const
{
    switch(_alignment)
    {
        ...
        ++titr)
    {
        ...
        //OSG_NOTICE<<"BigTextureText::computePositions("<<contextID<<") glyphquad= "<<&glyphquad<<std::endl;
...
void BigTextureText::computeBackdropPositions(unsigned int contextID) const
{
...
void BigTextureText::computeBackdropBoundingBox() const
{
    if(_backdropType == NONE)
    {
        return;
...
void BigTextureText::computeBoundingBoxMargin() const
{
...
void BigTextureText::computeColorGradients() const
{
...
void BigTextureText::computeColorGradientsOverall() const
{
...
void BigTextureText::computeColorGradientsPerCharacter() const
{
    for(TextureGlyphQuadMap::iterator titr=_textureGlyphQuadMap.begin();
        titr!=_textureGlyphQuadMap.end();
        ++titr)
...
void BigTextureText::drawImplementation(osg::RenderInfo& renderInfo) const
{
    drawImplementation(*renderInfo.getState(), osg::Vec4(1.0f,1.0f,1.0f,1.0f));
}

void BigTextureText::drawImplementation(osg::State& state, const osg::Vec4& colorMultiplier) const
{
    ...
    if (_drawMode & FILLEDBOUNDINGBOX)
    {
        ...
        #else
            OSG_NOTICE<<"Warning: BigTextureText::drawImplementation() fillMode FILLEDBOUNDINGBOX not supported"<<std::endl;
        #endif
        }
    }
...
void BigTextureText::accept(osg::Drawable::ConstAttributeFunctor& af) const
{
...
void BigTextureText::accept(osg::PrimitiveFunctor& pf) const
{
    for(TextureGlyphQuadMap::const_iterator titr=_textureGlyphQuadMap.begin();
        titr!=_textureGlyphQuadMap.end();
        ++titr)
...
void BigTextureText::setThreadSafeRefUnref(bool threadSafe)
{
...
void BigTextureText::resizeGLObjectBuffers(unsigned int maxSize)
{
...
void BigTextureText::releaseGLObjects(osg::State* state) const
{
...
void BigTextureText::setBackdropType(BackdropType type)
{
...
void BigTextureText::setBackdropImplementation(BackdropImplementation implementation)
{
...
void BigTextureText::setBackdropOffset(float offset)
{
...
void BigTextureText::setBackdropOffset(float horizontal, float vertical)
{
...
void BigTextureText::setBackdropColor(const osg::Vec4& color)
{
    _backdropColor = color;
}

void BigTextureText::setColorGradientMode(ColorGradientMode mode)
{
...
void BigTextureText::setColorGradientCorners(const osg::Vec4& topLeft, const osg::Vec4& bottomLeft, const osg::Vec4& bottomRight, const osg::Vec4& topRight)
{
...
float BigTextureText::bilinearInterpolate(float x1, float x2, float y1, float y2, float x, float y, float q11, float q12, float q21, float q22) const
{
    return (
...
void BigTextureText::drawForegroundText(osg::State& state, const GlyphQuads& glyphquad, const osg::Vec4& colorMultiplier) const
{
    unsigned int contextID = state.getContextID();
...
void BigTextureText::renderOnlyForegroundText(osg::State& state, const osg::Vec4& colorMultiplier) const
{
    for(TextureGlyphQuadMap::iterator titr=_textureGlyphQuadMap.begin();
        titr!=_textureGlyphQuadMap.end();
        ++titr)
...
void BigTextureText::renderWithDelayedDepthWrites(osg::State& state, const osg::Vec4& colorMultiplier) const
{
...
void BigTextureText::drawTextWithBackdrop(osg::State& state, const osg::Vec4& colorMultiplier) const
{
    unsigned int contextID = state.getContextID();
...
void BigTextureText::renderWithPolygonOffset(osg::State& state, const osg::Vec4& colorMultiplier) const
{
#if !defined(OSG_GLES1_AVAILABLE) && !defined(OSG_GLES2_AVAILABLE) && !defined(OSG_GL3_AVAILABLE)
    unsigned int contextID = state.getContextID();
    ...
    glPopAttrib();
#else
    OSG_NOTICE<<"Warning: BigTextureText::renderWithPolygonOffset(..) not implemented."<<std::endl;
#endif
}


void BigTextureText::renderWithNoDepthBuffer(osg::State& state, const osg::Vec4& colorMultiplier) const
{
#if !defined(OSG_GLES1_AVAILABLE) && !defined(OSG_GLES2_AVAILABLE) && !defined(OSG_GL3_AVAILABLE)
    unsigned int contextID = state.getContextID();
    ...
    glPopAttrib();
#else
    OSG_NOTICE<<"Warning: BigTextureText::renderWithNoDepthBuffer(..) not implemented."<<std::endl;
#endif
}

// This idea comes from Paul Martz's OpenGL FAQ: 13.050
void BigTextureText::renderWithDepthRange(osg::State& state, const osg::Vec4& colorMultiplier) const
{
#if !defined(OSG_GLES1_AVAILABLE) && !defined(OSG_GLES2_AVAILABLE) && !defined(OSG_GL3_AVAILABLE)
    unsigned int contextID = state.getContextID();
...
//    glPushAttrib(GL_VIEWPORT_BIT);
    ...
    glPopAttrib();
#else
    OSG_NOTICE<<"Warning: BigTextureText::renderWithDepthRange(..) not implemented."<<std::endl;
#endif
}

void BigTextureText::renderWithStencilBuffer(osg::State& state, const osg::Vec4& colorMultiplier) const
{
#if !defined(OSG_GLES1_AVAILABLE) && !defined(OSG_GLES2_AVAILABLE) && !defined(OSG_GL3_AVAILABLE)
    /* Here are the steps:
...
//    glDepthFunc(GL_ALWAYS);
    ...
    glPopAttrib();
#else
    OSG_NOTICE<<"Warning: BigTextureText::renderWithStencilBuffer(..) not implemented."<<std::endl;
#endif
}

BigTextureText::GlyphQuads::GlyphQuads()
{
    initGlyphQuads();
}

BigTextureText::GlyphQuads::GlyphQuads(const GlyphQuads&)
{
    initGlyphQuads();
}

void BigTextureText::GlyphQuads::initGlyphQuads()
{
...
void BigTextureText::GlyphQuads::updateQuadIndices()
{
...
void BigTextureText::GlyphQuads::initGPUBufferObjects()
{
...
void BigTextureText::GlyphQuads::resizeGLObjectBuffers(unsigned int maxSize)
{
...
void BigTextureText::GlyphQuads::releaseGLObjects(osg::State* state) const
{
    for (int j = 0; j < 8; j++)
    {
        for (size_t i = 0; i < _transformedBackdropCoords[j].size(); i++)
